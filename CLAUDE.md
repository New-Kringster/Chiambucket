...
- **`InfoModal.tsx`** (`'use client'`) — a small, theme-aware detail pop-up (`InfoItem`: eyebrow, title, blurb, optional add/points/chips/icon/link). CSS-only enter/exit, portaled to `body`, ESC/scrim/× to close. Accents follow the page's `data-theme` via `--hp-*` tokens (blue on home, violet on photography). Used by the homepage **capability cards** and the photography **"My Tools" cards** (each card is `role="button"` with a hover-rotating `+` affordance). Styles are `.im-*` in `mainstyle.css`.
- **`RollingWord.tsx`** (`'use client'`) — smooth word-cycle for the `.hp-roll` rotators ("Creating with Intention/Purpose/…", the closing CTA, and the Design Tools card's "equally at home in raster/vector/3D/motion."). Uses framer-motion (`AnimatePresence` + spring width) for a blurred cross-fade; renders an `<em>` so it inherits the surrounding context's gradient/font (`.hp-hero-title em`, `.hp-cta h2 em`, `.hp-tools-p .hp-roll em`). Takes `words`, optional `interval`/`startDelay` (to stagger multiple rolls on one page) and a static `ariaLabel` (screen readers shouldn't hear it change every cycle).
