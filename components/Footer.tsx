...
const SOCIALS = [
  {
    name: 'Instagram',
    href: 'https://www.instagram.com/bombastic_demise?igsh=MTQzZWQ0ODQyZmlyMg%3D%3D&utm_source=qr',
    fill: '#e1306c',
        ...
        <nav className="footer-social" aria-label="Social links">
          {SOCIALS.map((s) => (
            <a
              ...
              aria-label={s.name}
              style={{ '--brand': s.fill } as CSSProperties}
            >
              <svg viewBox={s.viewBox || '0 0 24 24'} xmlns="http://www.w3.org/2000/svg" fill={s.fill} aria-hidden="true">
                {s.icon}
              </svg>
              <span className="footer-social-tip" aria-hidden="true">{s.name}</span>
            </a>
          ))}
        </nav>
      </div>
