...
/* SpicyGate — dramatises BeadReader's per-reader "spicy" content gate: the
   same chapter paragraph rendered three ways depending on the reader's mode
   (blurred tap-to-reveal, locked, or entirely absent). Auto-cycles, pauses
   off-screen, never reflows the page. */

const LEAD = 'They had circled each other all evening. When the last guest finally left, the room went quiet, and ';
...
const MODES: Mode[] = [
  { key: 'full', label: 'Full access', caption: 'Tap the blurred line to reveal the spicy passage.' },
  { key: 'none', label: 'No access', caption: 'Locked. The passage never renders for this reader.' },
  { key: 'cal', label: 'Cal mode', caption: 'The passage is skipped entirely, no gap, no marker.' },
];
...
export default function SpicyGate() {
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => setModeIdx((p) => (p + 1) % MODES.length), 3400);
  ...
  const showRevealed = mode.key === 'full' && (revealed || reduced);
      ...
      <div ref={rootRef} className="sg" data-no-zoom>
        <div className="sg-panel">
          ...
          <div className="sg-page">
            <p className="sg-para">
              {LEAD}
              <span className="sg-passage-slot">
                <AnimatePresence mode="wait" initial={false}>
                  {mode.key === 'full' && (
                    <m.span
                      key="full"
                      className={`sg-passage full${showRevealed ? ' is-revealed' : ''}`}
                    ...
                    >
                      {!showRevealed && (
                        <button
                          ...
                          aria-label="Tap to reveal the spicy passage"
                        >
                          <span className="sg-blur-text" aria-hidden="true">{SPICY}</span>
                          <span className="sg-reveal-tag"><span aria-hidden="true">🌶</span> tap to reveal</span>
                        </button>
                      )}
                      {showRevealed && <span className="sg-passage-text">{SPICY}</span>}
                    </m.span>
                  )}
                  {mode.key === 'none' && (
                    <m.span
                      key="none"
                      className="sg-passage locked"
                      initial={reduced ? false : { opacity: 0 }}
                      animate={{ opacity: 1 }}
                      exit={reduced ? { opacity: 0 } : { opacity: 0 }}
                      transition={{ duration: 0.25 }}
                    >
                      <span className="sg-lock-block">
                        <span aria-hidden="true">🔒</span> Spicy passage, access on request
                      </span>
                    </m.span>
                  )}
                  {mode.key === 'cal' && (
        ...
        <style>{`
          ...
          .sg-page { min-height: 168px; display: flex; align-items: center; }
          .sg-para {
            font-family: 'dmsans'; font-size: 0.98rem; line-height: 1.75; color: rgba(240,232,220,0.86);
            margin: 0;
          }
          .sg-passage-slot { display: inline; }
          .sg-passage { display: inline; }
          .sg-passage.full { position: relative; }
          .sg-reveal-btn {
            position: relative; display: inline-flex; align-items: baseline; gap: 8px; flex-wrap: wrap;
            border: 0; padding: 0; margin: 0; background: none; cursor: pointer; text-align: left;
            font: inherit; color: inherit;
          }
          .sg-blur-text {
            filter: blur(6px); user-select: none; transition: filter 0.3s ease;
          }
          .sg-reveal-btn:hover .sg-blur-text { filter: blur(5px); }
          .sg-reveal-tag {
            display: inline-flex; align-items: center; gap: 5px; white-space: nowrap;
            font-family: 'inter'; font-weight: 700; font-size: 0.62rem; letter-spacing: 0.06em;
            text-transform: uppercase; color: var(--sg-spicy);
            background: rgba(224,102,61,0.14); border: 1px solid rgba(224,102,61,0.35);
            padding: 3px 9px; border-radius: 999px; vertical-align: middle;
          }
          .sg-reveal-btn:focus-visible { outline: 2px solid rgb(var(--sa-accent)); outline-offset: 3px; border-radius: 4px; }
          .sg-passage-text { color: var(--sg-spicy); }
          .sg-passage.locked { display: inline-block; }
          .sg-lock-block {
            display: inline-flex; align-items: center; gap: 7px; white-space: nowrap;
            font-family: 'inter'; font-weight: 600; font-size: 0.78rem; color: rgba(240,232,220,0.55);
            background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.12);
            padding: 4px 11px; border-radius: 8px; vertical-align: middle;
          }
          .sg-passage.cal { display: inline; }
          ...
          .sg-tab.is-active {
            color: #1a1208; background: rgb(var(--sa-accent)); border-color: rgb(var(--sa-accent));
          }
          ...
          .sg-footnote {
            font-family: 'dmsans'; font-size: 0.76rem; line-height: 1.5; color: rgba(232,220,200,0.45);
            max-width: 32ch;
          }
          ...
          @media (max-width: 420px) {
            .sg-page { min-height: 210px; }
            .sg-reveal-tag { display: inline-flex; }
          }
        `}</style>
      </div>
    </LazyMotion>
